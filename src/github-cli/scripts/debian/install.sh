...
get_gpg_key_servers() {
  ...
  # Assemble curl args in an array (rather than a string) so the arguments
  # reach curl as separate words. Expanding a quoted string would pass it
  # as one argument (or a blank one when empty that curl rejects outright).
  local curl_args=()
...
receive_gpg_keys() {
  ...
  # `keys` may hold several space-separated key IDs, so split them
  # into an array to be sure each one reaches gpg as a single argument
  local keys_array=()
  ...
  until [ "${gpg_ok}" = "true" ] || [ "${retry_count}" -eq "5" ]; do
    ...
    (printf '%s\n' "${keys_array[@]}" | xargs -n 1 gpg -q --recv-keys) 2>&1 &&
      gpg --export "${keys_array[@]}" | gpg --dearmor --yes -o "${keyring_path}" &&
      gpg_ok="true"
    if [ "${gpg_ok}" != "true" ]; then
...
# note: `find_version_from_git_tags` and `find_prev_version_from_git_tags` are provided by
# `../common.sh`, which the Alpine installer shares
...
# note: requested GitHub CLI extensions are installed by `../../install.sh` once this
# script returns, since none of that work is Debian-specific
