...
#-------------------------------------------------------------------------------------------------------------
#
# Docs: https://giopentofu/terraform.md
...
OPENTOFU_VERSION=${OPENTOFU:-"latest"}
...
OPENTOFU_SHA256="${OPENTOFU_SHA256:-"automatic"}"
...
# hopentofu.sigstore.dev/cosign/overview/
ensure_cosign() {
...
find_version_from_git_tags OPENTOFU_VERSION 'https://github.com/opentofu/opentofu'
...
# Install opentofu, tflint, Terragrunt
if [ "${INSTALL_OPENTOFU}" = "true" ]; then
    OPENTOFU_VERSION="latest"
    find_version_from_git_tags OPENTOFU_VERSION 'https://github.com/opentofu/opentofu'
    opentofu_filename="opentofu-v${OPENTOFU_VERSION}-linux-${architecture}.tar.gz"
    echo "(*) Downloading opentofu... ${opentofu_filename}"
    curl -sSL -o /tmp/tf-downloads/${opentofu_filename} https://github.com/opentofu/opentofu/releases/download/v${OPENTOFU_VERSION}/${opentofu_filename}
    if [ "${OPENTOFU_SHA256}" != "dev-mode" ]; then
        if [ "${OPENTOFU_SHA256}" = "automatic" ]; then
            curl -sSL -o opentofu_SHA256SUMS https://github.com/opentofu/opentofu/releases/download/v${OPENTOFU_VERSION}/opentofu-v${OPENTOFU_VERSION}.sha256sum
        else
            echo "${OPENTOFU_SHA256} *${tfsec_filename}" > opentofu_SHA256SUMS
        fi
        sha256sum --ignore-missing -c opentofu_SHA256SUMS
    fi
    mkdir -p /tmp/tf-downloads/opentofu
    tar -xzf /tmp/tf-downloads/${opentofu_filename} -C /tmp/tf-downloads/opentofu
    chmod a+x /tmp/tf-downloads/opentofu/opentofu
    mv -f /tmp/tf-downloads/opentofu/opentofu /usr/local/bin/tofu
fi
...
if [ "${INSTALL_SENTINEL}" = "true" ]; then
    ...
    sentinel_releases_url='https://releases.opentofu.com/sentinel'
