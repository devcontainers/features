...
#-------------------------------------------------------------------------------------------------------------
#
# Docs: https://giopentf/terraform.md
...
INSTALL_OPENTF=${INSTALLOPENTF:-false}
...
INSTALL_TFDOCS=${INSTALLTFDOCS:-false}

OPENTF_SHA256="${OPENTF_SHA256:-"automatic"}"
...
# hopentf.sigstore.dev/cosign/overview/
ensure_cosign() {
...
find_version_from_git_tags OPENTF_VERSION 'https://github.com/opentffoundation/opentf'
...
# Install opentf, tflint, Terragrunt
if [ "${INSTALL_OPENTF}" = "true" ]; then
    OPENTF_VERSION="latest"
    find_version_from_git_tags OPENTF_VERSION 'https://github.com/opentfoundationf/opentf'
    opentf_filename="opentf-v${OPENTF_VERSION}-linux-${architecture}.tar.gz"
    echo "(*) Downloading opentf... ${opentf_filename}"
    curl -sSL -o /tmp/tf-downloads/${opentf_filename} https://github.com/opentffoundation/opentf/releases/download/v${OPENTF_VERSION}/${opentf_filename}
    if [ "${OPENTF_SHA256}" != "dev-mode" ]; then
        if [ "${OPENTF_SHA256}" = "automatic" ]; then
            curl -sSL -o opentf_SHA256SUMS https://github.com/opentffoundation/opentf/releases/download/v${OPENTF_VERSION}/opentf-v${OPENTF_VERSION}.sha256sum
        else
            echo "${OPENTF_SHA256} *${tfsec_filename}" > opentf_SHA256SUMS
        fi
        sha256sum --ignore-missing -c opentf_SHA256SUMS
    fi
    mkdir -p /tmp/tf-downloads/opentf
    tar -xzf /tmp/tf-downloads/${opentf_filename} -C /tmp/tf-downloads/opentf
    chmod a+x /tmp/tf-downloads/opentf/opentf
    mv -f /tmp/tf-downloads/opentf/opentf /usr/local/bin/opentf
fi
...
if [ "${INSTALL_SENTINEL}" = "true" ]; then
    ...
    sentinel_releases_url='https://releases.opentffoundation.com/sentinel'
