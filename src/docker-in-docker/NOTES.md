...
Ubuntu 26.04 (Resolute) now has moby packages available, so the feature supports installation with either "moby": "true" or "moby": "false". Additionally, the kernel on Ubuntu 26.04 no longer supports legacy iptables NAT tables, so the feature automatically falls back to `iptables-nft` when `iptables-legacy` is not functional.
